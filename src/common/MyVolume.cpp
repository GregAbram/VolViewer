...
void
MyVolume:: SetTransferFunction(TransferFunction _tf) 
{
	if (! ospv)  return;
...
void 
MyVolume::Import(const std::string &filename, TransferFunction& tf)
{
  size_t x, y, z;
  std::string type;
  char rfile[256];
...
void
MyVolume::Attach(const std::string& type, int xsz, int ysz, int zsz, void *data, TransferFunction& tf)
{
	Initialize(true);
	SetType(type);
...
void
MyVolumeSeries::Import(const std::string &filename, TransferFunction& tf)
{
	...
	else
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			series[i].Import(vfile[0] == '/' || vfile[0] == '.' ? (dir + '/' + vfile).c_str() : vfile, tf);
...
void
MyVolumeSeries::ResetMinMax()
{
	for (std::vector<MyVolume>::iterator v = series.begin(); v != series.end(); ++v)
		v->ResetMinMax();
}

void
MyVolumeSeries::GetDimensions(int& _x, int& _y, int& _z)
{
	if (series.size() == 0)
	{
...
void
MyVolumeSeries::GetType(std::string& _t)
{
	if (series.size() == 0)
	{
...
void
MyVolumeSeries::GetSamplingRate(float& _r)
{
	if (series.size() == 0)
	{
...
void
MyVolumeSeries::GetMinMax(float& _m, float& _M)
{
	if (series.size() == 0)
	{
...
void
MyVolumeSeries::SetIsovalues(int n, float *v)
{
	if (series.size() == 0)
	{
	...
	for (std::vector<MyVolume>::iterator s = series.begin(); s != series.end(); ++s)
		s->SetIsovalues(n, v);
}

void
MyVolumeSeries::SetTransferFunction(TransferFunction _tf)
{
	if (series.size() == 0)
	{
